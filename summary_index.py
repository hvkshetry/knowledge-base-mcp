...
def upsert_summaries(
    ...
    for section_path, chunk_list in groups.items():
        ...
        cur.execute(
            """
            INSERT OR REPLACE INTO summaries(collection, doc_id, section_path, summary, element_ids)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
                collection,
...
def query_summaries(
    ...
    cur.execute(
        """
        SELECT doc_id, section_path, summary, element_ids
        FROM summaries
        WHERE collection = ?
    ...
    for doc_id, section_path, summary, element_ids in rows:
        results.append(
            {
                "doc_id": doc_id,
