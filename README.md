...
| Document summaries / outlines | ⚠️ Not generated by default | `kb.summary` / `kb.outline` return results only if summary/heading indexes are built manually |
| HyDE query expansion | ⚠️ Optional | `kb.hyde` returns content only if a local text-generation model is configured (via `OLLAMA_LLM`) |
